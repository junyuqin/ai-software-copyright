...
from utils.data_fusion import calc_3d_scores
...
def create_app(config_class=None):
    ...
    # 注册路由
    register_routes(app, llm_service)
...
def register_routes(app, llm_service):
    """注册所有路由"""
    
    @app.route('/health', methods=['GET'])
    def health_check():
        """健康检查接口，用于 Docker 健康检测"""
    ...
    def get_profile(uuid):
        ...
        """
        try:
            # 模拟从数据库获取行为数据（实际应从 behavior_data 表查询）
            mock_data = {
                "quiz_avg": 75.0,
                "attendance": 0.9,
                "debug_success": 0.65,
